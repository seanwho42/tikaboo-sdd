...
# filtered for clones
filtered_pedigree = read_table('franz-files/pedigree.txt') %>% clean_names() %>% 
...
```{r}
pedigree2 = read_table('franz-files/6m-2/pedigree.txt') %>% clean_names() %>% 
  # added selfed column for ease later on with visualization and such
  mutate(selfed = (sire == dam) & !is.na(dam)) %>%
  filter(sire!='*') %>%
  mutate(dam=na_if(dam, '*'))
...
```
Let's also read in the results of our pedigree analysis without filtering for clones in genodive
```{r}
unfiltered_pedigree <- read_table('franz-files/unfiltered/6min-100b-1/pedigree.txt')
head(unfiltered_pedigree)
```



comparing analyzing differences between pedigree and pedigree2
```{r}
differences_pedigree = full_join(pedigree, pedigree2, by='id') %>% 
  filter(sire.x != sire.y)
differences_pedigree

no_longer_existing_pedigree = full_join(pedigree, pedigree2, by='id') %>%
  filter(is.na(`sire.x`))
no_longer_existing_pedigree

new_pedigree = no_longer_existing_pedigree = full_join(pedigree, pedigree2, by='id') %>%
  filter(is.na(`sire.y`))
new_pedigree
```
...
parentage_lods <- read_csv("franz-files/6m-2/parentage.csv") %>% clean_names()
...
```
Inferred parents for 163 offsprin
```{r}
#pedigree2 for now
...
#   # cutting out parent type to get distinct because otherwise selfed will show up twice (sire, dam)
#   select(-parent_type) %>% 
#   distinct()

filtered_franz <- inner_join(pedigree2, filtered_parentage_lods, by = c("id" = "offspring", "sire" = "parent_1", "dam" = "parent_2")) %>%
  rename('Tree' = 'id') %>%
  # making parent type sire or dam for now, but could be overwritten with max, min, selfed, etc
  gather(key='parent_type', value='parent_id', sire, dam) %>%
  # cutting out parent type to get distinct because otherwise selfed will show up twice (sire, dam)
...
distances_m <- distances_m %>% filter(offspring_id != "some_id") %>% arrange(desc(distance))
