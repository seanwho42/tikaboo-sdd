...
Before we get started on our formatting, FRANz won't work properly with clones in our data, so let's get rid of these.

First we can check for clones by looking for instances where all of the data is identical (apart from the tree id)
```{r}
franz_genotypes_slashes %>%
  ...
  arrange(yb01,yb15,yb21,yb04,Tree)

genotypes %>% arrange(`yb05 Allele#1`, `yb05 Allele#2`, `yb15 Allele#1`, `yb15 Allele#2`, `yb21 Allele#1`, `yb21 Allele#1`)
```
...
```{r}
# using geno_coords because we only care about trees which we have coordinates for.
franz_genotypes <- geno_coords %>%
...
Now, lets actually remove these clones. We're doing this pseudorandomly.
```{r}
...
FRANz --Nmax 60000 --selfing --poutformat 2 --pedigreeoutformat 1,2,3 --hwetestout hwe-out --mhsamplefreq 1000 --mhburnin 50000000 --mhiter 100000000000 --samaxiter 10000000000 --mintyped 6 franz_cloneless.dat

```

--Nmax 60000 estimates that the maximum number of potential parents is 10000
...
--hwetestout generates a HWE test with our results
...
--mhsamplefreq makes the sampling for log files less frequent so the generated files aren't massive
--mhburnin 50000000 designates 50000000 iterations for metropolis-hastings MCMC burnin
--mhiter 100000000000 designates 100000000000 iterations for metropolis-hastings MCMC
--samaxiter 10000000000 designates 10000000000 maximum iterations for simulated annealing
--mintyped 6 specifies 6 minimum typed loci (this is the default value, but there was some trial and error here)
franz_cloneless.dat is our input file
